...
enum {
    STATE_REQ = 0,
    STATE_RES = 1,
    STATE_END = 2, // marks the connection for deletion
};

struct Conn {
    int fd = -1;
    uint32_t state = 0; // either STATE_REQ or STATE_RES
    // Added: read buffer offset (index of the beginning of valid data)
    size_t rbuf_offset = 0;
    // Number of valid data bytes from the offset onward
    size_t rbuf_size = 0;
...
static void *begin_arr(std::string &out) {
    out.push_back(SER_ARR);
    out.append("\0\0\0\0", 4); // will be filled in end_arr()
    return (void *)(out.size() - 4); // use as context pointer
}
...
static void entry_set_ttl(Entry *ent, int64_t ttl_ms) {
    if (ttl_ms < 0 && ent->heap_idx != (size_t)-1) {
        // Erase an item from the heap by replacing with the last item.
...
static bool try_one_request(Conn *conn) {
    ...
    // Pointer to the beginning of valid data in the buffer
    uint8_t *buf = conn->rbuf + conn->rbuf_offset;
    uint32_t len = 0;
    memcpy(&len, buf, 4);
    if (len > k_max_msg) {
        msg("too long");
    ...
    if (4 + len > conn->rbuf_size) {
        return false; // Not enough data available to process the request
    }
    std::vector<std::string> cmd;
    if (0 != parse_req(buf + 4, len, cmd)) {
        msg("bad req");
        conn->state = STATE_END;
        return false;
    }
    std::string out;
    out.reserve(128);  // Pre-reserve to reduce small allocations
    do_request(cmd, out);
    ...
    // Advance the offset by the processed request length
    conn->rbuf_offset += 4 + len;
    conn->rbuf_size -= 4 + len;
    // If the entire buffer is processed, reset the offset to 0
    if (conn->rbuf_size == 0) {
        conn->rbuf_offset = 0;
    }
...
static bool try_fill_buffer(Conn *conn) {
    // If there is not enough space in the buffer to read new data, 
    // shift the unprocessed data to the beginning of the buffer (only when necessary)
    if (conn->rbuf_offset > 0 && (sizeof(conn->rbuf) - (conn->rbuf_offset + conn->rbuf_size)) < 1024) {
        memmove(conn->rbuf, conn->rbuf + conn->rbuf_offset, conn->rbuf_size);
        conn->rbuf_offset = 0;
    }
    assert(conn->rbuf_offset + conn->rbuf_size <= sizeof(conn->rbuf));
    ssize_t rv = 0;
    do {
        size_t cap = sizeof(conn->rbuf) - (conn->rbuf_offset + conn->rbuf_size);
        rv = read(conn->fd, conn->rbuf + conn->rbuf_offset + conn->rbuf_size, cap);
...
static bool try_flush_buffer(Conn *conn) {
    ...
    // Some data remains in wbuf, try writing again
    return true;
}
...
static void connection_io(Conn *conn) {
    // Wake-up by poll, update the idle timer by moving the connection to the end of the list.
    conn->idle_start = get_monotonic_usec();
    dlist_detach(&conn->idle_list);
    dlist_insert_before(&g_data.idle_list, &conn->idle_list);

    // Process I/O based on current state
...
static uint32_t next_timer_ms() {
    ...
    if (next_us == (uint64_t)-1) {
        return 10000; // No timer, value doesn't really matter
    }
...
static void process_timers() {
    // Add extra 1000us for poll()'s ms resolution
    ...
    while (!g_data.heap.empty() && g_data.heap[0].val < now_us) {
        ...
        if (nworks++ >= k_max_works) {
            // Avoid stalling the server if too many keys expire at once
            break;
        }
    }
}
...
int main() {
    ...
    // --- Optimization: Pre-allocate poll_args vector capacity ---
    std::vector<struct pollfd> poll_args;
    poll_args.reserve(g_data.fd2conn.size() + 1);
    // ----------------------------------------------------------------

    // Event loop
    while (true) {
        poll_args.clear();
        // Reserve capacity each loop to avoid repeated reallocations
        poll_args.reserve(g_data.fd2conn.size() + 1);

        // The listening fd is in the first position for convenience
        ...
        // Process active connections (skip listening fd at index 0)
        for (size_t i = 1; i < poll_args.size(); ++i) {
            if (poll_args[i].revents) {
                Conn *conn = g_data.fd2conn[poll_args[i].fd];
                connection_io(conn);
                if (conn->state == STATE_END) {
                    // Client closed normally, or an error occurred.
