...
// ompare by the (score, name) tuple
static bool zless(
...
static void zeset_update(ZSet *zset, ZNode *node, double score) {
    if (node->score == score) {
        return;
    }
...
// offset into the succeeeding or preceding node.
